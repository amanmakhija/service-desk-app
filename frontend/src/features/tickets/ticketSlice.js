...
export const closeTicket = createAsyncThunk(
  'tickets/close',
  async ({ ticketId, closingUser, closedByUser }, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token
      return await ticketService.closeTicket(ticketId, token, closingUser, closedByUser)
    } catch (error) {
      return thunkAPI.rejectWithValue(extractErrorMessage(error))
    }
  }
...
export const updateTicket = createAsyncThunk(
  'tickets/update',
  async ({ ticketId, assignedUser, assignedName }, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token
      return await ticketService.updateTicket(ticketId, token, assignedUser, assignedName)
    } catch (error) {
      return thunkAPI.rejectWithValue(extractErrorMessage(error))
    }
  }
